<?php
	...
	class Provider{
		...
		public  function createClient($key){
		$this->CI =& get_instance();
		$this->CI->load->model('client_model');
		//$key = sha1(OAuthProvider::generateToken(20,true));
		$secret = OAuthProvider::generateToken(20,false);
			
		$this->CI->client_model->create($key,$secret);
		return $secret; 
		...
		public function checkConsumer($provider){
			...
			$aClient = Client::findByKey($provider->consumer_key);

			if(is_object($aClient)){
				if(!aClient->isActive(aclient){
					$return = OAUTH_CONSUMER_KEY_REFUSED;
				} else {
					$this->consumer = $aClient;
					$provider->consumer_secret = $this->aClient->getSecret();
					$return = OAUTH_OK;
				}
			}
